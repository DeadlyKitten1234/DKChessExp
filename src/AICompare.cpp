#include "AICompare.h"
const int AICompare::MAX_PLY_MATE = 256;
const int AICompare::CHECK_BONUS = 125;
const int AICompare::MAX_HISTORY = 120;
const int AICompare::COUNTER_MOVE_BONUS = 192;
const int AICompare::KILLER_BONUS = 350;
const int AICompare::NULL_MOVE_DEFEND_BONUS = 75;
const int AICompare::NULL_MOVE_MATE_DEFEND_BONUS = 225;
...
void AICompare::initPos(Position* pos_) {
	pos = pos_;
	resetHistory();
}

int16_t AICompare::startSearch(uint64_t timeToSearch) {
...
void AICompare::updateHistoryNewSearch() {
	for (int8_t i = 0; i < 6; i++) {
		for (int8_t j = 0; j < 64; j++) {
...
inline int16_t AICompare::iterativeDeepening(int8_t depth) {
...
inline int16_t AICompare::search(int8_t depth, int16_t alpha, int16_t beta) {
	if (Clock::now() >= searchEndTime) {
		return 0;
	}
	if (depth <= 0) {
	...
	bool failLow = 1, mateIfNullMove = 0;
	...
	prefetch((const char*)&ttCmp.chunk[pos->zHash >> ttCmp.shRVal].entry);
	//Prefetch if no en passant
	if (pos->m_possibleEnPassant != -1) {
		prefetch((const char*)&ttCmp.chunk[(pos->getZHashNoEp()) >> ttCmp.shRVal].entry);
	}
	...
	if (!pvNode && !pos->friendlyInCheck && pos->hasNonPawnPiece(pos->m_blackToMove) && !abCloseToMate && eval >= beta) {
		...
		if (inNullMoveSearch == lastWasNull) {
			...
			TTEntry* nullMoveTTE = ttCmp.find(pos->zHash, foundNullMoveTTE);
			if (foundNullMoveTTE) {
	...
	if ((ttBestMove == nullMove || !pos->isEnPassant(ttBestMove)) && foundTTEntryNoEp && ttEntryNoEp->bestMove != nullMove &&
			...
			if (res >= beta) {
				if (++curCutNodes == cutNodesToQuit) {
					...
					if (pos->m_possibleEnPassant != -1 && !pos->isEnPassant(curMove)) {
						//ttEntryNoEp->init(pos->getZHashNoEp(), curMove, res, depth - multiCutDepthR, ttCmp.gen, LowBound);
					}
					return res;
				}
			}
	...
	if (!pvNode && depth > probCutDepthR && !abCloseToMate &&
		...
		for (int16_t i = 0; i < movesCnt; i++) {
			...
			if (res >= probCutBeta) {
				...
				if (pos->m_possibleEnPassant != -1 && !pos->isEnPassant(curMove)) {
					//ttEntryNoEp->init(pos->getZHashNoEp(), curMove, res, depth - probCutDepthR, ttCmp.gen, LowBound);
				}
				return res;
			}
		}
	...
	for (int16_t i = 0; i < movesCnt; i++) {
		...
		if (res >= beta) {
			...
			if (pos->m_possibleEnPassant != -1 && !pos->isEnPassant(curMove)) {
				//ttEntryNoEp->init(pos->getZHashNoEp(), curMove, res, depth, ttCmp.gen, BoundType::LowBound);
			}
			if (root) {
	...
	if (pos->m_possibleEnPassant != -1 && !(pos->isEnPassant(curBestMove))) {
		//ttEntryNoEp->init(pos->getZHashNoEp(), curBestMove, bestValue, depth, ttCmp.gen, (failLow ? BoundType::HighBound : BoundType::Exact));
	}
	return alpha;
}

//https://www.chessprogramming.org/Quiescence_Search
inline int16_t AICompare::searchOnlyCaptures(int16_t alpha, int16_t beta) {
	if (Clock::now() >= searchEndTime) {
		return 0;
	}
	...
	//When going to cut, don't ttCmp.find, because its expensive
	if (staticEval >= beta) {
		return staticEval;
	}
	...
	//Probe tt
	bool foundTTEntry = 0;
	TTEntry* ttEntryRes = ttCmp.find(pos->zHash, foundTTEntry);
	...
	for (int16_t i = 0; i < movesCnt; i++) {
		if (i != movesCnt - 1) {
			prefetch((const char*)&ttCmp.chunk[pos->getZHashIfMoveMade(moves[moveIndices[i + 1]]) >> ttCmp.shRVal]);
		}
...
inline void AICompare::orderMoves(int16_t* moves, int16_t movesCnt, int16_t* indices,
