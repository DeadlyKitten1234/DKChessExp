...
const int AICompare::NULL_MOVE_DEFEND_BONUS = 30;
const int AICompare::NULL_MOVE_MATE_DEFEND_BONUS = 420;
...
int16_t AICompare::startSearch(uint64_t timeToSearch, bool printRes) {
...
inline int16_t AICompare::iterativeDeepening(int8_t depth, bool printRes) {
	bool foundRootInTT = false;
	TTEntry* ttRoot = ttCmp.find(pos->zHash, foundRootInTT);
			...
			if (nullDefenders[pt] & (1ULL << endPos)) {
				if (mateIfNullMove) {
					...
					if (!nmIsBigCapture && nmIsCapture) {
						...
						const int bonus = min(300, (floorLog2(x) * fastSqrt(x) / (floorLog2(x) + fastSqrt(x))) * x / 34);
						curGuess += bonus;
					}
				}
			}
