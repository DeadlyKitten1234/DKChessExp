...
void populateMovesLookup() {
	for (int i = 0; i < 64; i++) {
		...
		for (int j = 0; j < (1ULL << rookRelevantSqCnt[i]); j++) {
			...
			if (rookMovesLookup[i][getMagicIdx(blockBitmask, i, 0)] != 0) {
				throw ("Rook lookup magic isn't correct on sq " + i);
			}
		...
		for (int j = 0; j < (1ULL << bishopRelevantSqCnt[i]); j++) {
			...
			if (bishopMovesLookup[i][getMagicIdx(blockBitmask, i, 1)] != 0) {
				throw ("Bishop lookup magic isn't correct on sq " + i);
			}
